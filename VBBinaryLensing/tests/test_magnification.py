...
def test_BinSourceLightCurveParallax():
    ...
    VBBL.SetObjectCoordinates("./VBBinaryLensing/data/OB151212coords.txt","./VBBinaryLensing/data/satellite1.txt")
    mag = VBBL.BinSourceLightCurveParallax([1,np.log10(0.5),0.01,0.025,10,15,0.1,-0.1],[10.25,2,69],[0.018],[0.0035])
...
def test_BinSourceLightCurveXallarap():
    ...
    mag = VBBL.BinSourceLightCurveXallarap([1,0.5,0.01,0.025,10,15,0.1,-0.1,0.6,0.2,0.3,0.5],[10.25,2,69],[0.018],[0.0035],[10.0,6,7])
...
def test_BinaryLightCurve():
    ...
    assert np.allclose(mag,[14.989374837435403, 1.0842731547424023, 1.0])



def test_BinaryLightCurveOrbital():

    mag =  VBBL.BinaryLightCurveOrbital([np.log10(0.97),-1.5,0.01,0.1,-2.5,1.5,10,-0.25,0.36,0.1,0.056,0.03],[10.25,2,69],[0.018],[0.0035],[1,25,36])

    
    assert np.allclose(mag,[14.446493529090516, 1.0811430891964906, 1.0])

def test_BinaryLightCurveParallax():

    mag =  VBBL.BinaryLightCurveParallax([np.log10(0.97),-1.5,0.01,0.1,-2.5,1.5,10,0.6,0.025],[10.25,2,69],[0.018],[0.0035])

    assert np.allclose(mag,[14.846407903118143, 1.0831311541007225, 1.0])
...
def test_BinaryLightCurveW():
    ...
    assert np.allclose(mag,[15.92662349677546, 1.0732162742327906, 1.0])
...
def test_BinaryMag():
    ...
    assert np.abs(mag-4.36339307985976)<tol
...
def test_ESPLLightCurve():
    ...
    assert np.allclose(mag,[5.762272891302062, 5.762147624422236, 1.7530582089918916])
...
def test_ESPLMag():
    ...
    assert magnification ==  40.012478065951136
...
def test_ESPLMag2():
    ...
    assert magnification ==  44.17330135905762
...
def test_ESPLMagDark():
    ...
    rho = 0. 
    VBBL.LoadESPLTable("./VBBinaryLensing/data/ESPL.tbl")
    magnification = VBBL.ESPLMagDark(u, rho,a1)
    ...
    assert magnification ==  44.17171567310498
